...
```ascii
┌────────────────────────────────────────────────────┐
│ > ESP32 Marauder + Custom Jammer (2.4/5GHz full)   │
│   → WiFi/Bluetooth DEAUTH storm + Evil Portal      │
│   → Packet injection ao vivo no celular            │
│   → BadUSB payload integrado                       │
└────────────────────────────────────────────────────┘
```    
...
┌─────────────────────────────────────────────────────┐
│ > Python & automações                               │
│   → Automatização de tudo Existente                 │
│                                                     │
│ > Red Team full cycle (OSCP | OSCE | CRTP holder)   │
│   → Do recon ao domain admin                        │
│                                                     │
│ > Exploits 0day privados                            │
│   → No meu Discord talvez tenha algo...             │
│                                                     │
│ > Reverse engineering (x64dbg + Ghidra = amor)      │
│   → Puro Passa-Tempo                                │
│                                                     │
│ > Web pwn (XSS → RCE)                               │
│   → SQLi → shell → pivô → café enquanto limpo logs  │
│                                                     │
│ > Active Directory blood rituals com BloodHound     │
│   → Path to DA mais curto que fila de pão           │
│                                                     │
│ > Bots de todos os sabores e pecados                │
│   → Telegram, Discord, IG, OnlyFans, Tinder, etc    │
│   → Se tem API, já tem meu bot dominando,           │
│      se não tem API, eu dou um jeito.               │
└─────────────────────────────────────────────────────┘
```
...
![Detran RJ](https://img.shields.io/badge/Detran_RJ-consulta_placa_gr%C3%A1tis_via_bot_24h-orange?style=flat-square&logo=telegram)
![Light S.A.](https://img.shields.io/badge/Light-medidores_inteligentes_433MHz_sniffados-cyan?style=flat-square)
